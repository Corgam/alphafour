...
from agents.helpers import BoardPiece, PLAYER1, NO_PLAYER
...
def encode_board(board: np.ndarray, player: BoardPiece = PLAYER1):
    '''
    ...
    encoder_dict = {1: 1, "O": 2}
    # Move the values from the board
    for row in range(6):
        for col in range(7):
            if board[row][col] != NO_PLAYER:
                neuralBoard[row, col, board[row, col] - 1] = 1
    # Set the player to move
    neuralBoard[:][:][2] = player - 1
    return neuralBoard